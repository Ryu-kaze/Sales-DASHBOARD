...
def generate_gemini_insights(df, forecast_df, mape, product_name):
    """Call Gemini and parse structured JSON insights."""
...
with tab2:
    ...
    if "gemini_result" not in st.session_state or st.session_state.get("gemini_product") != selected_product:
        with st.spinner("🤖 Gemini is analysing your sales data..."):
            try:
                import google.generativeai as genai
                genai.configure(api_key=GEMINI_API_KEY)
                test = genai.GenerativeModel("gemini-2.0-flash")
                st.session_state["gemini_result"] = generate_gemini_insights(df, forecast_df, mape, selected_product)
                st.session_state["gemini_product"] = selected_product
                st.session_state["gemini_error"] = None
            except Exception as e:
                st.session_state["gemini_result"] = None
                st.session_state["gemini_error"] = str(e)
    gemini_result = st.session_state.get("gemini_result")
    gemini_error  = st.session_state.get("gemini_error")
    ...
    if gemini_result:
        ...
        # Regenerate button
        st.markdown("<br>")
        if st.button("🔄 Regenerate Insights", help="Ask Gemini again"):
            for k in ["gemini_result", "gemini_product", "gemini_error"]:
        ...
        if gemini_error:
            ...
            with st.expander("🔍 Debug Info"):
                st.code(f"API Key (first 10 chars): {GEMINI_API_KEY[:10]}...
Error: {gemini_error}")
        else:
            st.warning("⚠️ Gemini returned no response. Check your API key and internet access.")
        if st.button("🔄 Retry Gemini"):
            for k in ["gemini_result", "gemini_product", "gemini_error"]:
                st.session_state.pop(k, None)
            st.rerun()
        st.markdown("---")
        st.markdown("**Showing built-in insights as fallback:**")
        total_units = df["units_sold"].sum()
